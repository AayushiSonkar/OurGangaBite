...
def index(request):
    return HttpResponse("THIS IS MY APP INDEX (test)")
...
def dashboard(request):
    #  Step 1: Agar user login nahi hai to login page pe redirect karo
    if not request.user.is_authenticated:
        return HttpResponseRedirect('/login')

    context = {}

    #  Step 2: Safe fetch for logged-in user and profile
    login_user = get_object_or_404(User, id=request.user.id)
    profile, created = Profile.objects.get_or_create(user=login_user)
